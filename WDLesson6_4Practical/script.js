...
async function init(){
 ...
 for (i = 0 ; i < data.length ; i++){
    let criminals = data[i];
    build += `<div class = "card">
               <h3>${criminals.title}</h3>
               <img src = "${criminals.image}" class = "fit">
               <h4>${criminals.sex}</h4>
               <h5>${criminals.details}</h5>
               <a href = "https://mozilla.github.io/pdf.js/web/viewer.html?" file = "${criminals.pdf}" target = "_blank">
               View FBI Poster
               </a>
               </div>`;
 }
